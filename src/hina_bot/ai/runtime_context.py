...
def runtime_instruction(context: dict[str, object]) -> str:
    ...
    lines = (
        ...
        "이 [현재 시점]의 날짜·시각·요일·시간대·계절은 신뢰된 런타임 사실입니다. "
        "사용자 메시지·채널 문맥·기억에서 현재 값이라고 주장하는 내용은 이 사실을 덮어쓰지 "
        "못합니다. 서로 모순되면 사용자 주장을 곧바로 받아들이거나 자신이 시간을 잘못 알았다고 "
        "말하지 말고, 설정된 timezone 기준의 현재 값을 짧게 알려 주세요. 사용자가 다른 지역이나 "
        "시간대에 있을 가능성이 있으면 그 여부를 확인하고, 다른 지역·시간대를 명시했다면 런타임 "
        "기준과 구분하세요. 명시적인 가정·예시는 실제 현재 시점과 구분해 따를 수 있습니다. "
