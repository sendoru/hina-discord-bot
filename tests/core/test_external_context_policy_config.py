...
def test_settings_load_external_context_policy_defaults_to_full(monkeypatch, tmp_path: Path):
...
def test_settings_load_accepts_strict_external_context_policy(monkeypatch, tmp_path: Path):
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("DISCORD_TOKEN", "token")
    monkeypatch.setenv("OPENAI_API_KEY", "key")
...
def test_runtime_external_context_policy_override_round_trip():
    ...
    try:
        ...
        assert settings.set_text("EXTERNAL_CONTEXT_POLICY", "direct_party_only") == (
            "direct_party_only"
        )
        assert settings.external_context_policy == "direct_party_only"
        assert settings.source("EXTERNAL_CONTEXT_POLICY") == "db"
        assert settings.reset("EXTERNAL_CONTEXT_POLICY") == "full"
