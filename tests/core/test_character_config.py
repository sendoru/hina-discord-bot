...
def test_character_config_drives_retrieval_terms(monkeypatch):
    ...
    # Character identifiers are noise, but world/organization names stay searchable.
    assert LoreIndex._terms("아리스야 밀레니엄 전투") == {"밀레니엄", "전투"}
    assert runtime_terms("아리스 밀레니엄 전투") == {"밀레니엄", "전투"}
...
def test_knowledge_candidate_scoring_ignores_character_only_match(monkeypatch):
    ...
    row = {
        ...
        "keywords": ["아리스", "밀레니엄", "레일건"],
        "subjects": ["아리스", "밀레니엄"],
    }

    assert _row_score("아리스", row) == 0
    assert _row_score("밀레니엄 소속이야?", row) > 0
