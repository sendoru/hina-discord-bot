...
현재 호출 메시지, 사용자가 명시적으로 답장한 메시지, 그리고 제한된 최근 채널 메시지의 시각
입력을 모델에 전달할 수 있습니다.
...
- 이미지가 있는 메시지에 답장한 후속 질문
- `/chatlog`가 켜진 채널에서 `아까 그 사진`, `저거` 같은 후속 질문
...
- Discord embed와 일반 URL을 따라가 이미지로 해석
- PDF/문서 파일 해석
- 이미지 생성·편집
- 이미지 원본/caption을 장기 기억에 자동 저장
- Lottie 스티커

최근 이미지는 현재 질문에 직접 첨부하지 않고, 원래 메시지의 본문·작성자·message ID와 묶인
과거 시각 문맥으로 시간순 배치합니다. 모델은 현재 질문의 답장 대상과 대화 흐름이 연결되는 경우에만
이를 사용하며, 무관하거나 관계가 불명확하면 답변에서 완전히 무시하도록 지시받습니다.
...
부여되며 같은 값이 usage 로그에도 기록됩니다. 세 structured telemetry 파일의 각 row에는
`app_version`, `build_revision`, `runtime_id`도 함께 기록합니다. `build_revision`은 명시적으로
주입한 `HINA_GIT_SHA` 또는 checkout의 Git HEAD를 우선 사용하고, Git metadata가 없는 이미지에서는
